...
- 章の追加、削除、並び替え、タイトル編集
- Kindle / しまうまA6塗り足し版 / しまうまA5塗り足し版 / しまうまA6漫画塗り足し版 / しまうまA5漫画塗り足し版 / カスタムのページプリセット
- 余白、本文サイズ、ルビサイズ、行間、段落間隔の調整
- ルビ、画像、QRカード、改ページの挿入
- QRリンクライブラリ
- 挿絵画像はIndexedDBに分離保存（本文HTMLと自動保存を軽量に保ち、入力の引っかかりを防ぐ）
- ブラウザ内の自動保存
- JSONインポート / エクスポート（画像はassetsとしてdata URI同梱）
- 現在の原稿レイアウトをPDFファイルとして書き出し（ブラウザ本来の描画を使うため編集画面と折り返し・ルビが一致）
- Kindle向けの簡易DOCX出力
- Google Drive保存の接続口
